...
export const getPreviousDonations = async (req, res) => {
  try {
    const result = await donationService.getPreviousDonations(req.query);
    ...
    sendResponse(res, 200, true, 'Donations fetched successfully', { count: result.donations.length, page: result.pageNum, donations: result.donations });
  } catch (error) {
    console.error('Error fetching previous donations:', error);
    sendResponse(res, 500, false, 'Server error while fetching previous donations');
  }
};

export const getDonationsByUser = async (req, res) => {
  try {
    const results = await donationService.getDonationsByUser();
    sendResponse(res, 200, true, 'Donations fetched successfully', { count: results.length, donations: results });
  } catch (error) {
    console.error('Error aggregating donations by user:', error);
    sendResponse(res, 500, false, 'Server error while aggregating donations by user');
  }
};
...
import { logDonationCreated, logDonationFailed } from '../services/auditLogService.js';
...
// Start a session for transaction
  ...
  try {
    ...
    // Validation
    if (!causeId || !amount) {
      return sendResponse(res, 400, false, 'Cause ID and amount are required');
    }

    if (amount <= 0) {
      return sendResponse(res, 400, false, 'Donation amount must be greater than 0');
    }
    ...
    if (platformConfig.minimumDonation.enabled && amount < platformConfig.minimumDonation.amount) {
      return sendResponse(res, 400, false, `Donation amount must be at least ${platformConfig.currency.symbol}${platformConfig.minimumDonation.amount}`);
    }

    // Find the cause (outside transaction for validation)
    const cause = await Cause.findById(causeId);
    if (!cause) {
      return sendResponse(res, 404, false, 'Cause not found');
    }

    // Check if cause is active
    if (cause.status !== 'active') {
      return sendResponse(res, 400, false, `This cause is currently ${cause.status} and not accepting donations`);
    }

    // Check if cause has ended
    if (cause.endDate && new Date(cause.endDate) < new Date()) {
      return sendResponse(res, 400, false, 'This cause has ended and is no longer accepting donations');
    }
...
export const makeMultiDonation = async (req, res) => {
const session = await mongoose.startSession();
  ...
  try {
    ...
    if (!causes || !Array.isArray(causes) || causes.length === 0) {
      return sendResponse(res, 400, false, 'At least one cause must be selected');
    }

    if (!totalAmount || totalAmount <= 0) {
      return sendResponse(res, 400, false, 'Total donation amount must be greater than 0');
    }
    ...
    if (platformConfig.minimumDonation.enabled && totalAmount < platformConfig.minimumDonation.amount) {
      return sendResponse(res, 400, false, `Total donation amount must be at least ${platformConfig.currency.symbol}${platformConfig.minimumDonation.amount}`);
    }
    ...
    if (Math.abs(totalAllocated - totalAmount) > 0.01) { // Allow for floating point errors
      return sendResponse(res, 400, false, `Allocated amounts (${totalAllocated}) must equal total amount (${totalAmount})`);
    }

    // Validate each cause allocation
    for (const causeAllocation of causes) {
      if (!causeAllocation.causeId || !causeAllocation.amount) {
        return sendResponse(res, 400, false, 'Each cause must have a valid ID and amount');
      }

      if (causeAllocation.amount <= 0) {
        return sendResponse(res, 400, false, 'Each cause allocation must be greater than 0');
      }
    }
    ...
    if (causeDocs.length !== causeIds.length) {
      return sendResponse(res, 404, false, 'One or more causes not found');
    }

    // Check if all causes are active
    const inactiveCauses = causeDocs.filter(c => c.status !== 'active');
    if (inactiveCauses.length > 0) {
      return sendResponse(res, 400, false, `Some causes are not active: ${inactiveCauses.map(c => c.name).join(', ')}`);
    }

    // Check for ended causes
    const endedCauses = causeDocs.filter(c => c.endDate && new Date(c.endDate) < new Date());
    if (endedCauses.length > 0) {
      return sendResponse(res, 400, false, `Some causes have ended: ${endedCauses.map(c => c.name).join(', ')}`);
    }
...
try {
    ...
    sendResponse(res, 500, false, 'Server error while fetching donation history');
  }
...
try {
    ...
    sendResponse(res, 500, false, 'Server error while fetching donation statistics');
  }
...
export const getDonationReceipt = async (req, res) => {
  ...
  try {
    ...
    if (req.user && req.user.role === 'admin') {
      ...
      donation = user.donations.find(d => d.paymentId === paymentId);
      if (!donation) {
        return sendResponse(res, 404, false, 'Donation not found for this user');
      }
    }
    ...
    sendResponse(res, 500, false, 'Failed to generate receipt');
  }
